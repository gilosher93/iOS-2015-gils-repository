...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    /* example of inserting new rows/data to db */
    ...
    /* example of updating existing row/data in db */
    func updateDataInDatabase(){
        let batch = NSBatchUpdateRequest(entityName: entityName);
        batch.propertiesToUpdate = ["age" : 18];
        batch.predicate = NSPredicate(format: "age > %@", 16 as NSNumber);
        batch.resultType = .UpdatedObjectsCountResultType;
        do{
            let result = try managedObjectContext.executeRequest(batch) //batch inherit from NSPersistentStoreRequest
    ...
    /* example of reading existing rows/data from db */
    ...
    /* example of deleting existing rows/data fro db */
    func deleteFromDb(){
        let fetchRequest = NSFetchRequest(entityName: entityName);
        ...
        do{
            ...
            if let theLastPerson = lastPerson{
                managedObjectContext.deleteObject(theLastPerson);
            }
            try managedObjectContext.save();
        }catch{
    ...
    lazy var applicationDocumentsDirectory: NSURL = {
        // The directory the application uses to store the Core Data store file. This code uses a directory named "example.Learning_Core_Data" in the application's documents Application Support directory.
